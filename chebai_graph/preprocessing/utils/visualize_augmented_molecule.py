...
def _create_graph(edge_index, augmented_graph_nodes, augmented_graph_edges):
    G = nx.Graph()
    node_labels = {}
    node_colors = []
    node_type_map = {}
    ...
    for atom in atom_nodes.GetAtoms():
        ...
        node_colors.append(NODE_COLOR_MAP["atom"])
    ...
    for fg_idx, fg_props in fg_nodes.items():
        ...
        node_colors.append(NODE_COLOR_MAP["fg"])
    ...
    node_colors.append(NODE_COLOR_MAP["graph"])
...
def _draw_hierarchy(
    G, mol, node_labels, node_type_map, edges, edge_colors, fg_ids, graph_ids
):
    ...
    fg_pos = {
        ...
        for node, (x, y) in nx.spring_layout(G.subgraph(fg_ids), seed=42).items()
    }
    graph_pos = {
        node: (x, y + max_atom_pos_y + 3)
        for node, (x, y) in nx.spring_layout(G.subgraph(graph_ids), seed=123).items()
    }

    pos = {**atom_pos, **fg_pos, **graph_pos}
    node_colors = [NODE_COLOR_MAP[node_type_map[n]] for n in G.nodes]
...
def _draw_simple(G, node_labels, node_colors):
    plt.figure(figsize=(10, 8))
...
def _draw_3d(G, mol, node_labels, node_type_map, edges, fg_ids, graph_ids):
    try:
    ...
    node_trace = go.Scatter3d(
        ...
        marker=dict(
            size=8,
            color=[NODE_COLOR_MAP[node_type_map[n]] for n in G.nodes],
            opacity=0.9,
        ),
...
def plot_augmented_graph(
    ...
    (
        ...
        fg_ids,
        graph_ids,
    ) = _create_graph(edge_index, augmented_graph_nodes, augmented_graph_edges)
    ...
    if plot_type == "h":
        ...
        _draw_simple(G, node_labels, node_colors)
    elif plot_type == "3d":
        _draw_3d(G, mol, node_labels, node_type_map, edges, fg_ids, graph_ids)
    else:
